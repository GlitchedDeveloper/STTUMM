namespace STTUMM
{
    //Original code: https://github.com/SigmaDolphin/SkylanderEditor/blob/master/CRCcalculator.vb
    //Translated by: GlitchedGamer
    public class CRCcalculator
    {
        public static long[] CRCTab = new long[256]; //Array for single byte CRCs loaded from table

        //swap from little endian to big endian and viceversa
        public static long SwapEndianness(byte byte1, byte byte2)
        {
            string bytebuffer;

            bytebuffer = (byte1 < 0x10 ? "0" : "") + byte1.ToString("X");
            bytebuffer = byte2.ToString("X") + bytebuffer;
            bytebuffer = Convert.ToInt32(bytebuffer, 16).ToString();
            return long.Parse(bytebuffer);
        }

        //https://github.com/mandar1jn/portfolio/blob/master/src/scripts/skylanders/skylanders.ts
        private static string crc16(byte[] data)
        {
            ...
            return (crc & 0xFFFF).ToString("X").PadLeft(4, '0');
        }

        //calculate the checksums of a skylander by type
        public static string checksumCalc(int chkType, int sector, byte[] skylanderData)
        {
            int chksum1 = 0, chksum2 = 0, chksum3 = 0, chksum4 = 0;
            ...
            switch (chkType)
            {
                ...
                case 4:
                    ...
                    b = crc16(crcbuffer);
                    b = (Convert.ToInt64(b, 16) ^ 0xE953).ToString("X");
                    b = b.PadLeft(4, '0');
                    b = b.Substring(b.Length - 4);
                    return b.Substring(b.Length - 2) + b.Substring(0, 2);
                default:
                    crcbuffer = [];
                    break;
            }
            Console.WriteLine(crc16(crcbuffer));
            b = crc16(crcbuffer);
            b = b.Substring(b.Length - 4);
            return b.Substring(b.Length - 2) + b.Substring(0, 2);
        }
